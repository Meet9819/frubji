...
$exists_query = "SELECT COUNT(*) AS `count` FROM `productsprice` WHERE `productid` = ? AND `variantid` = ? AND `branchid` = ?";
$update_query = "UPDATE `productsprice` SET `price` = ? WHERE `productid` = ? AND `variantid` = ? AND `branchid` = ?";
$insert_query = "INSERT INTO `productsprice`(`productid`, `variantid`, `branchid`, `price`) VALUES (?, ?, ?, ?)";
...
try {
    ...
    if (empty($price_master)) {
        ...
        echo json_encode(array(
            "status" => false,
            "message" => "Invalid update price master data!",
        ));
        return;
    }
