...
$status = false;
if (authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["create"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["edit"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["view"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["delete"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["approval"])) {
    $status = true;
}
     ...
     <!-- Title -->
        <div class="row heading-bg">
          <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h5 class="txt-dark">Branch Request
            </h5>
          </div>
          <!-- Breadcrumb -->
        ...
        //  <!-- ------------------------------------------------------------------------------------------------------------------------------------------- -->
          ...
          if (isset($_POST["create_invoice"])) {
              ...
              $statement = $connect->prepare("INSERT INTO   br_request_order
                                      (requestorderprifix,branchreferenceno, requestorder_date, requestfrom, requestto,  order_datetime,remarks,whichcompany) VALUES (:requestorderprifix,:branchreferenceno, :requestorder_date, :requestfrom, :requestto, :order_datetime,:remarks,:whichcompany)
                                  ");
              $statement->execute(
                  array(
              ...
              for ($count = 0; $count < $_POST["total_item"]; $count++) {
                  ...
                  $statement = $connect->prepare("
                                        INSERT INTO   br_request_order_item
                                        (order_id, itemcode, item_name,units, order_item_quantity, requestfromm,requesttoo,whichcompany)
                                        VALUES (:order_id, :itemcode,:item_name,:units, :order_item_quantity,:requestfromm,:requesttoo,:whichcompany)
                                      ");
              ...
              ?>
             <script>
             alert('New Branch Request added successfully!');
             window.location.href='branchrequest.php';
             </script>
             <?php
...
if (isset($_POST["update_invoice"])) {
    ...
    $statement = $connect->prepare("DELETE FROM br_request_order_item WHERE order_id = :order_id");
    $statement->execute(
        array(
            ':order_id' => $order_id,
        )
    ...
    ?>
                <script>
                alert('Branch Request updated successfully!');
                window.location.href='branchrequest.php';
                </script>
                <?php
}
...
if (isset($_GET["delete"]) && isset($_GET["id"])) {
    $statement = $connect->prepare("DELETE FROM   br_request_order WHERE order_id = :id");
    $statement->execute(
        array(
            ':id' => $_GET["id"],
        )
    ...
    $statement = $connect->prepare(
        "DELETE FROM  br_request_order_item WHERE order_id = :id");
    $statement->execute(
        array(
            ':id' => $_GET["id"],
        )
    ...
    ?>
     <script>
     alert('Branch Request Successfully Deleted !!');
     window.location.href='branchrequest.php';
     </script>
     <?php
}
    ...
    ?>
                    ...
                    <form method="post" id="invoice_form">
                      <div class="table-responsive" >
                        ...
                        <div class="from-group row">
                                 ...
                                 <?php    
                                 include "db.php";
                                 $result = mysqli_query($con, "SELECT order_id FROM br_request_order ORDER BY order_id desc LIMIT 1");
                                     ...
                                     <b>Request To  </b><br />
                                      <select autofocus="" name="requestto"  id="requestto"  class="form-control "  data-placeholder="Choose a Branch Request " tabindex="0">
                                           <option selected="" value="">Select Branch Request to</option>
                                            <?php
                                            include "db.php";
                                            $result = mysqli_query($con, "SELECT branchcode,branchname_english FROM `branch` where branchcode != '$workingin'");
                                              while ($row = mysqli_fetch_array($result)) {
                                                  echo '<option value="' . $row['branchcode'] . '">' . $row['branchcode'] . ' - ' . $row['branchname_english'] . '</option>';
                                              }
                                              ?>
                                      </select>
                                  </div>
              ...
              <div class="table-responsive" style="overflow-y: auto;overflow-x: hidden;height: 45%">
                  ...
                  <table id="invoice-item-table" class="table table-bordered">
                    <tr>
                      ...
                      <th style="padding: 10px" width="5%">Qty</th>  <th style="padding: 10px" width="7%">Units</th>   
                      <th  width="3%"></th>
                    </tr>
                    ...
                    <div class="row" style="padding: 10px 0;">
                      <div class="col-md-12 text-center">
                        <input type="hidden" name="total_item" id="total_item" value="1" />
                         <a href="branchrequest.php" class="btn btn-danger btn-rounded btn-lable-wrap left-label"><span class="btn-label"><i class="fa fa-times"></i> </span><span class="btn-text">Close</span> </a>
      ...
      $(document).ready(function(){
        ...
        function addrow(){
          ...
          html_code += '<td> <button type="button" name="remove_row" id="'+count+'" class="btn btn-danger btn-icon-anim btn-circle remove_row"><i class="fa fa-trash" aria-hidden="true"></i></button></td>';
...
<?php
    ...
    $statement = $connect->prepare(" SELECT * FROM   br_request_order WHERE order_id = :order_id LIMIT 1");
    $statement->execute(
        array(
            ':order_id' => $_GET["id"],
        )
        ...
        <form method="post" id="invoice_form">
                  ...
                  <table id="invoice-item-table" class="table table-bordered">
                    ...
                    <tr>
                      ...
                      <td>
                        <button type="button" name="remove_row" id="'+count+'" class="btn btn-danger btn-icon-anim btn-circle remove_row"><i class="fa fa-trash" aria-hidden="true"></i></button>
                      </td>
                    </tr>
                    <?php }?>
                  </table>
                   ...
                   <div class="col-md-12"> <input type="text" name="remarks" id="remarks" data-srno="1" class="form-control input-sm remarks" placeholder="Enter the Remarks to be displayed in your Branch Request Document">
                   </div>
...
<!-- ------------------------------------------------------------------------------------------------------------------------------------------- -->
 ...
 <!-- VIEW code form design  -->
    ...
    <!-- ROLE BASED CREATE-->
    <?php if (authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["create"])) {?>

        <div class="text-right">
        <a href="branchrequest.php?add=1" class="btn btn-primary  btn-icon right-icon"><i class="fa fa-plus"></i><span class="btn-text"> Create Branch Request</span></a> </div><br>
    <?php }?>
    <!-- ROLE BASED CREATE-->
        <thead>
          <tr>
        ...
        <?php 
          ...
          include 'database_connection.php';
          $statement = $connect->prepare("SELECT b.order_id, b.whichcompany,b.requestorderprifix,b.requestorder_date,b.requestfrom,b.requestto, bb.branchname_english,b.order_datetime,b.remarks,b.status,b.branchreferenceno FROM `br_request_order` b, `branch` bb where b.requestto = bb.branchcode ORDER BY b.order_id DESC");
          $statement->execute();
          ...
          foreach ($all_result as $row) {
            ...
            echo '
              <tr>
                ...
                <td>' . $row["branchname_english"] . '</td>
                <td>';$requestorderdate = $row['requestorder_date'];
                echo $newrequestorderdate = date("d-m-Y", strtotime($requestorderdate));echo '</td>
               
              ';?>

               <?php include 'modal/viewbranchrequest.php';?>
              <TD>

                <!-- ROLE BASED VIEW -->
                           <?php if (authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["view"])) {?>

                          <?php echo '
                          <a target="_blank" href="printpdf/print_branchrequest.php?pdf=1&id=' . $row["order_id"] . '" ><i class="fa fa-file-text-o txt-default"> </i> </a> &nbsp;
                          '; ?>
                ...
                <!-- ROLE BASED EDIT -->
                                <?php if (authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["edit"])) {?>

                                           <?php
if ($row['status'] == 0) {
                echo '
                                           <a href="branchrequest.php?update=1&id=' . $row["order_id"] . '"  class="text-inverse pr-10"> <i class="zmdi zmdi-edit txt-success">
                                                      </i></a> ';
            } else {
                ...
                <!-- ROLE BASED DELETE -->
                                ...
                                <?php if (authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["delete"])) {?>
              ...
              <!-- ROLE BASED APPROVAL -->
                                ...
                                <?php if (authorize($_SESSION["access"]["PURCHASE"]["BRANCH_REQUEST"]["approval"])) {?>
...
?>
 ...
 <!-- VIEW code form design  end-->
        ...
        if(confirm("Are you sure to "+ msg)){
          ...
          url = "allstatus/branchrequestajax.php";
          $.ajax({
          type:"POST",
          url: url,
            ...
            <script type="text/javascript">
              $(document).ready(function(){
                ...
                $(document).on('click', '.delete', function(){
                  ...
                  {
                    window.location.href="branchrequest.php?delete=1&id="+id;
                  }
                  else
                  {
                    return false;
