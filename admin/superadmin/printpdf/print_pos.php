...
if(isset($_GET["pdf"]) && isset($_GET["id"]))
{
 ...
 $statement = $connect->prepare("
  SELECT p.order_id,p.whichcompany,p.requestorderprifix, p.requestorder_date,p.requestfrom, b.branchname_english, p.customername,p.typeofsale,p.order_total_before_tax, p.order_total_after_tax, p.order_datetime,p.remarks,p.status,p.mobile,p.updated_on FROM `pos` p, `branch` b where p.requestfrom = b.id and  
  p.order_id = :order_id
  LIMIT 1
 ");
 $statement->execute(
 ...
 foreach($result as $row)
 {
  $output .= '
    <table width="100%" border="0" cellpadding="5" cellspacing="0" >  
     ...
     ';
  $statement = $connect->prepare(
   "SELECT pi.order_item_id,pi.order_id,pi.itemcode,pi.item_name,pi.variant,pv.qty, pv.units, pi.order_item_quantity,pi.order_item_price,pi.order_item_actual_amount,pi.order_item_final_amount FROM pos_item pi, productvariant pv where pi.variant = pv.id and pi.order_id = :order_id"
  );
  $statement->execute(
   array(
    ':order_id' =>  $_GET["id"]
