...
$status = FALSE;
if ( authorize($_SESSION["access"]["MASTER"]["ECOMMERCEORDERSDELIVERYBOY"]["create"]) || 
authorize($_SESSION["access"]["MASTER"]["ECOMMERCEORDERSDELIVERYBOY"]["edit"]) || 
authorize($_SESSION["access"]["MASTER"]["ECOMMERCEORDERSDELIVERYBOY"]["view"]) || 
authorize($_SESSION["access"]["MASTER"]["ECOMMERCEORDERSDELIVERYBOY"]["delete"]) || 
authorize($_SESSION["access"]["MASTER"]["ECOMMERCEORDERSDELIVERYBOY"]["approval"])) {
 $status = TRUE;
}
...
<div class="row heading-bg">
  <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
    <h5 class="txt-dark">Ecommerce Orders Master Delivery Boy
    </h5>
  </div>
  <!-- Breadcrumb -->
  <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
    <ol class="breadcrumb">
      ...
      <li class="active">
        <span> Ecommerce Orders Master Delivery Boy
        </span>
      </li>
    </ol>
  </div>
...
<table id="datatable" data-toggle="table">
      ...
      include('db.php');
              ...
              <tr>         
                  <td>'.$row['id'].'</td>                
                  <td>'.$row['representativeid'].' - '.$row['firstname'].'</td>         
                  <td>'.$row['billno'].'</td>                     
                  ...
                  <td>'.$row['deliveryboy'].'</td>
                 <td> 

                                <a  href="ecommerceordersdetaildeliveryboy.php?edit_id='.$row['id'].'&billno='.$row['billno'].'" class="text-inverse" data-toggle="tooltip"><i style="background-color:#ff24ee;color:black" class="fa fa-eye btn  btn-sm waves-effect waves-light"></i></a> &nbsp;&nbsp;
