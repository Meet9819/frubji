...
foreach($result as $row)
{
	...
	if($row["image"] != '')
	{
		$image = '<img src="ecommerce/blogsimages/'.$row["image"].'" class="img-thumbnail" width="250px" height="auto" />';
	}
	else
	{
		$image = '';
	...
	$sub_array[] = $row["title"];
	$sub_array[] = substr($row['shortdescription'],0,100).'....';   
	$sub_array[] = substr($row['description'],0,150).'....'; 
	$sub_array[] = $row["datee"];
