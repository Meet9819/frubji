...
$status = false;
if (authorize($_SESSION["access"]["PURCHASE"]["POS"]["create"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["POS"]["edit"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["POS"]["view"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["POS"]["delete"]) ||
    authorize($_SESSION["access"]["PURCHASE"]["POS"]["approval"])) {
    $status = true;
}
     ...
     <!-- Title -->
        ...
        //  <!-- ------------------------------------------------------------------------------------------------------------------------------------------- -->
          ...
          if (isset($_POST["create_invoice"])) {
              ...
              $statement = $connect->prepare("INSERT INTO   pos
                                      (requestorderprifix,branchreferenceno, requestorder_date, requestfrom, customername, order_total_before_tax,  order_total_after_tax, order_datetime,remarks,whichcompany,typeofsale) VALUES (:requestorderprifix,:branchreferenceno, :requestorder_date, :requestfrom, :customername, :order_total_before_tax, :order_total_after_tax, :order_datetime,:remarks,:whichcompany,:typeofsale)
                                  ");
              $statement->execute(
                  array(
              ...
              for ($count = 0; $count < $_POST["total_item"]; $count++) {
                  ...
                  $statement = $connect->prepare("
                                        ...
                                        (order_id, itemcode, item_name,units, order_item_quantity, order_item_price, order_item_actual_amount, order_item_final_amount,requestfromm,customernamee,whichcompany)
                                        VALUES (:order_id, :itemcode,:item_name,:units, :order_item_quantity, :order_item_price, :order_item_actual_amount, :order_item_final_amount,:requestfromm,:customernamee,:whichcompany)
                                      ");
                  ...
                  $statement->execute(
                      array(
                          ...
                          ':customernamee' => trim($_POST["customername"]),
              ...
              ?>
             <script>
             alert('New POS added successfully!');
...
if (isset($_POST["update_invoice"])) {
    ...
    $statement->execute(
        array(
            ...
            ':order_id' => $order_id,
            ':order_datetime' => $currentTime,
            ':typeofsale' => trim($_POST["typeofsale"]),
        )
...
if (isset($_GET["delete"]) && isset($_GET["id"])) {
    $statement = $connect->prepare("DELETE FROM   pos WHERE order_id = :id");
    $statement->execute(
        array(
            ':id' => $_GET["id"],
        )
    ...
    $statement = $connect->prepare(
        "DELETE FROM  pos_item WHERE order_id = :id");
    $statement->execute(
        array(
            ':id' => $_GET["id"],
        )
...
if (isset($_GET["add"])) { //insert code form design
    ?>
                    ...
                    <form method="post" id="invoice_form">
                      <div class="table-responsive" >
                        ...
                        <div class="from-group row">
                                 ...
                                 <?php    
                                 include "db.php";
                                 $result = mysqli_query($con, "SELECT order_id FROM pos ORDER BY order_id desc LIMIT 1");
                                 ...
                                 <div class="col-md-2 form-group">
                                  ...
                                  <div class="col-md-2">
                                      <b>Type of Sale </b><br />
                                        <select name="typeofsale" id="typeofsale" class="form-control">
                                ...
                                Customer Mobile No<br />
                                  <input  type="text" name="branchreferenceno" id="branchreferenceno" class="form-control input-sm" placeholder="Mobile No." tabindex="0"  />
                              </div>

                               <div class="col-md-1">
                               Date <br />
                              <input type="text" name="requestorder_date" id="requestorder_date" class="form-control input-sm"  placeholder="Select POS Date"  value="<?php echo $fulldatee; ?>" readonly="" tabindex="-1" />
                              </div>
              ...
              <div class="table-responsive" style="overflow-y: auto;overflow-x: hidden;height: 45%">
                  ...
                  <div class="col-md-8">
                    <input type="text" name="remarks" id="remarks" data-srno="1" class="form-control input-sm remarks" placeholder="Enter the Remarks to be displayed in your POS Document">
                  </div>
                    ...
                    <div class="row" style="padding: 10px 0;">
                      <div class="col-md-12 text-center">
                        <input type="hidden" name="total_item" id="total_item" value="1" />
                         <a href="pos.php" class="btn btn-danger btn-rounded btn-lable-wrap left-label"><span class="btn-label"><i class="fa fa-times"></i> </span><span class="btn-text">Close</span> </a>
...
<?php
    ...
    $statement = $connect->prepare(" SELECT * FROM   pos WHERE order_id = :order_id LIMIT 1");
    $statement->execute(
        array(
            ':order_id' => $_GET["id"],
        )
    ...
    foreach ($result as $row) {
        ...
        <form method="post" id="invoice_form">
                  <div class="row">
                      ...
                      <div class="form-group col-md-2"><b>Customer Name </b><br />
                        <input type="text" name="customername" id="customername" class="form-control input-sm" placeholder=" Br. Ref. No"  />
                      </div>
  ...
  <div class="form-group col-md-1">Ref. No<br />
                  ...
                  <table id="invoice-item-table" class="table table-bordered">
                    ...
                    <?php 
                    $statement = $connect->prepare("SELECT * FROM   pos_item WHERE order_id = :order_id");
                    $statement->execute(
                        array(
                            ':order_id' => $_GET["id"],
                        )
                   ...
                   <div class="col-md-8"> <input type="text" name="remarks" id="remarks" data-srno="1" class="form-control input-sm remarks" placeholder="Enter the Remarks to be displayed in your POS Document">
                   </div>
...
<!-- ------------------------------------------------------------------------------------------------------------------------------------------- -->
 ...
 <!-- VIEW code form design  -->
    ...
    <!-- ROLE BASED CREATE-->
    <?php if (authorize($_SESSION["access"]["PURCHASE"]["POS"]["create"])) {?>

        <div class="text-right">
        <a href="pos.php?add=1" class="btn btn-primary  btn-icon right-icon"><i class="fa fa-plus"></i><span class="btn-text"> Create POS</span></a> </div><br>
    <?php }?>
    <!-- ROLE BASED CREATE-->
        <thead>
          <tr>
            ...
            <th width="20px">Document No </th>
            <th  width="10px">Customer Name </th>
            <th>Request Date</th>
            <th> Amount</th>
            <th>Permission</th>
            <th>Stage</th>
        ...
        <?php 
          ...
          include 'database_connection.php';
          $statement = $connect->prepare("SELECT b.order_id, b.whichcompany,b.requestorderprifix,b.requestorder_date,b.requestfrom,b.customername, b.order_total_before_tax,b.order_total_after_tax,b.order_datetime,b.remarks,b.status,b.branchreferenceno FROM `pos` b ORDER BY b.order_id DESC");
          $statement->execute();
          ...
          foreach ($all_result as $row) {
            ...
            echo '
              ...
              ';?>
               ...
               <?php include 'modal/viewpos.php';?>
              <TD>

                <!-- ROLE BASED VIEW -->
                           <?php if (authorize($_SESSION["access"]["PURCHASE"]["POS"]["view"])) {?>

                          <?php echo '
                          <a target="_blank" href="printpdf/print_pos.php?pdf=1&id=' . $row["order_id"] . '" ><i class="fa fa-file-text-o txt-default"> </i> </a> &nbsp;
                          '; ?>
                ...
                <!-- ROLE BASED EDIT -->
                                <?php if (authorize($_SESSION["access"]["PURCHASE"]["POS"]["edit"])) {?>

                                           <?php
if ($row['status'] == 0) {
                echo '
                                           <a href="pos.php?update=1&id=' . $row["order_id"] . '"  class="text-inverse pr-10"> <i class="zmdi zmdi-edit txt-success">
                                                      </i></a> ';
            } else {
                ...
                <!-- ROLE BASED DELETE -->
                                ...
                                <?php if (authorize($_SESSION["access"]["PURCHASE"]["POS"]["delete"])) {?>
             ...
             <?php if (authorize($_SESSION["access"]["PURCHASE"]["POS"]["approval"])) {?>
            ...
            <script type="text/javascript">
              $(document).ready(function(){
                ...
                $(document).on('click', '.delete', function(){
                  ...
                  {
                    window.location.href="pos.php?delete=1&id="+id;
                  }
                  else
                  {
                    return false;
...
<script>
  ...
  $('#invoice-item-table').on("input", ".datalist-input", function() {
    ...
    if (!$('#requestfrom').val()) {
      swal("", "Please Select POS from!","warning");
      $('#requestfrom').focus();
      return;
    }

    if (!$('#customername').val()) {
      swal("", "Please Select POS To","warning");
      $('#customername').focus();
      return;
    }
