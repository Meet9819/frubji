...
$(document).ready(() => {
    ...
    if (!queryParams.has('add') && !queryParams.has('edit')) {
        populateITEM_MASTEROverviewtable(supplierCompany, supplierBranch);
    }
...
function getITEM_MASTEROrder() {
    try {
        let posOrderFormData = new FormData($('#order-invoice-form')[0]);
        posOrderFormData.append('totalInvoiceItems', totalOrderItems);
        posOrderFormData.append('invoiceTotal', invoiceFinalTotal);
        posOrderFormData.append('workingin', supplierBranch);
        posOrderFormData.append('orderNo', orderNo);
        return posOrderFormData;
    } catch (error) {
        return null;
    }
}
...
function deleteITEM_MASTEROrder(rowNo, orderNo) {
    try {
        swal({
            ...
            .then((confirm) => {
                if (confirm) {
                    return fetch(`${baseURL}/API/item_master/deleteorder.php?orderNo=${orderNo}`, {
                        method: 'GET',
                    });
                } else {
                    throw null;
...
function populateITEM_MASTEROverviewtable(company) {
    try {
        let requestData = {
            'company': company
        ...
        getOverviewTableData(`${baseURL}/API/item_master/getorders.php`, requestData)
            .then((res) => {
                            ...
                            'render': function(data, type, row, meta) {
                                ...
                                if (type === 'display') {
                                    ...
                                    if (editAccess && !orderPosted) {
                                        rowHTML += `<a href="itemaddedit.php?edit_id=${data}" title="Edit order" class="text-inverse" style="margin: 0 5px;"><i class="zmdi zmdi-edit txt-success"></i></a>`;
                                    }

                                    if (deleteAccess && !orderPosted) {
                                        rowHTML += `<a href="#" title="Delete order" onclick="deleteITEM_MASTEROrder(${rowNo}, ${data})" class="delete" style="margin: 0 5px;"><i class="zmdi zmdi-delete txt-danger" style="margin: 5px;"></i></a>`;
                                    }
