          ...
          <div id="primary" class="content-area col-md-12">
            <?php 
              ...
              if (empty($_SESSION["branch"]["price"]))  {
                $result = mysqli_query($con,"SELECT `id` AS `product_id`, `name` AS `product_name`, `img` AS `image`, `shortdescription` AS `desc_short` FROM `products` WHERE (`categoryid` = {$pr_id} OR `maincat` = {$pr_id}) AND `status` = 1 ORDER BY `name` ASC");
                $products = mysqli_fetch_all($result, MYSQLI_ASSOC);
              } else {
                foreach ($_SESSION["branch"]["price"] as $key => $value) {
                  if (($pr_id == $value["product_cat_id"]) || ($pr_id == $value["product_cat"])) {
                  ...
                  <div class="xs-product-widget media xs-md-20">
                    ...
                    <div class="media-body align-self-center product-widget-content">
                      <h4 class="product-title">
                        <a href="<?php echo "detailpage.php?q={$product_id}"; ?>"><?php echo $product_name; ?></a>
                      </h4>                    
                    </div>
                  </div>
                </div>
                    ...
                    <div class="xs-product-wraper text-center">
                     ...
                     ?>  <img style="height:140px" class="attachment-woocommerce_thumbnail size-woocommerce_thumbnail wp-post-image" alt="<?php echo $product_name; ?>" src="media/products/<?php echo "$img"; ?>" />  <?php 
                      ...
                      <ul class="product-item-meta">
                       ...
                       <?php 
                        ...
                        <?php 
                              ...
                              ?>
                            <li class="xs-cart-wrapper">
                              <a href="login.php" type="button" title="Add to cart" data-quantity="1" class="button product_type_variable add_to_cart_button" data-product_id="22"  rel="nofollow">Add to cart</a>
                            </li>
                      ...
                      <div class="xs-product-content">
                        ...
                        <?php if (!empty($selected_variant_price) && !empty($_SESSION["branch"]["pin"]) && ($selected_variant_price > 0)) { ?>
                        <span class="price">
